package com.pku.leetcode.tree;

/**
 * Created by zhaolizhen on 18-3-11.
 */

/**
 * Given two binary trees and imagine that when you put one of them to cover the other, some nodes of the two trees are overlapped while the others are not.

 You need to merge them into a new binary tree. The merge rule is that if two nodes overlap, then sum node values up as the new value of the merged node. Otherwise, the NOT null node will be used as the node of new tree.

 Example 1:
 Input:
 Tree 1                     Tree 2
 1                         2
 / \                       / \
 3   2                     1   3
 /                           \   \
 5                             4   7
 Output:
 Merged tree:
 3
 / \
 4   5
 / \   \
 5   4   7
 Note: The merging process must start from the root nodes of both trees.
 */
public class Merge_Two_Binary_Trees_617 {

    public static void main(String args[]){
        Solution solution=new Solution();
        TreeNode t1=TreeUtils.createTree();
        TreeNode t2=TreeUtils.createTree();
        TreeNode treeNode=solution.mergeTrees(t1, t2);
        TreeUtils.printTree(treeNode);
    }

    private static class Solution1 {
        public TreeNode mergeTrees(TreeNode t1, TreeNode t2) {
            if(null==t1&&t2==null){
                return null;
            }

            TreeNode root=new TreeNode(0);
            if(null==t1&&null!=t2){
                root.val=t2.val;
                root.left=mergeTrees(t1,t2.left);
                root.right=mergeTrees(t1,t2.right);
            }

            if(null!=t1&&null==t2){
                root.val=t1.val;
                root.left=mergeTrees(t1.left,t2);
                root.right=mergeTrees(t1.right,t2);
            }

            if(null!=t1&&null!=t2){
                root.val=t1.val+t2.val;
                root.left=mergeTrees(t1.left,t2.left);
                root.right=mergeTrees(t1.right,t2.right);
            }

            return root;
        }
    }

    //Clear and simple way to solve this problem
    //the origin structure is broken
    // from https://leetcode.com/problems/merge-two-binary-trees/solution/
    private static class Solution {
        public TreeNode mergeTrees(TreeNode t1, TreeNode t2) {
            if (t1 == null)
                return t2;
            if (t2 == null)
                return t1;
            t1.val += t2.val;
            t1.left = mergeTrees(t1.left, t2.left);
            t1.right = mergeTrees(t1.right, t2.right);
            return t1;
        }
    }
}
